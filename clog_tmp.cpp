 
//#include "circuit.hpp"
//#include "node.hpp"

void circuit::log_sim(string log_file_name){
    char tmp;

    //set val of all Node to -1 (initial value of each node in circuit is -1)
    for (int i = 0; i < Nnodes; i++){
        np = Node[i];
        np -> val = -1;
    }
    

    //read the primary input file, if number(in input file) = Pinput -> num, val = value(in input file)
    fin.open(log_file_name, ios::in);
    
    while (getline(fin, log_in)) {
        stringstream col(log_in);
       
        col>>log_num;
        col>>tmp;
        col>>log_val;
       // cout<<"log_num = "<<log_num<<endl;
        for(int i = 0; i < Npi; i++){
            np = Pinput[i];
            //cout<<"num = "<<np->num<<endl;
            if(np->num == log_num){
                np->val = log_val;
                break;
               // cout << "Primary num and val is "<<np->num<<","<<np->val<<endl;
            }// if read file line successful
        } // for
    } // while
    fin.close();
   
    // find the max lvl from all Poutput
    int max_lvl = 0;
    int sum = 0; //in xor gate
    for (int i = 0; i < Npo; i++){
        np = Poutput[i];
        if (np->lvl > max_lvl){
            max_lvl = np->lvl;
        }
    }
    //cout<<"max_lvl = "<<max_lvl<<endl;
       
    // for all node which lvl is j (from 1 to max lvl)
        // if type = 1 (branch)  val = unodes[0] -> val
        // if type = 6 (NAND)  if undoes[i(from 0 to fin-1)]=0, val = 1, otherwise val =0 (control value = 0)
        // ...... other type
    for (int j = 1; j <= max_lvl; j++){ // lev = 1, 2 ... max_lev
        //cout<<"level is:"<<j<<endl;
        for(int i = 0; i < Nnodes; i++){
            np = Node[i];
            if (np->lvl == j){
                switch(np->type){
                        
                    case (e_gtype::BRCH):
                        np->val = np->unodes[0]->val;
                        //cout<<"it's branck"<<endl;
                        break;
                    
                    case (e_gtype::XOR):
                        sum = 0;
                        for (int k = 0 ; k < np->fin; k ++){
                            sum = sum ^ np->unodes[k]->val;
                        }
                            np->val = sum;
                        //cout<<"it's XOR"<<endl;
                        break;
                        
                    case  (e_gtype::OR):
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 1){
                                np->val = 1;
                                break;
                            }
                            else np->val = 0;
                        } // for k
                        //cout<<"it's OR"<<endl;
                        break;
                        
                    case  (e_gtype::NOR):
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 1){
                                np->val = 0;
                                break;
                            }
                            else np->val = 1;
                        } // for k
                        //cout<<"it's NOR"<<endl;
                        break;
                    
                    case  (e_gtype::NOT):
                        np->val = !(np->unodes[0]->val);
                        //cout<<"it's NOT"<<endl;
                        break;
       
                    case (e_gtype::NAND):
                        //cout<<"it's NAND"<<endl;
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 0){
                                //cout<<"unodes:"<<np->unodes[k]->num<<": "<<np->unodes[k]->val<<endl;
                                np->val = 1;

                                break;
                            }
                            else np->val = 0;
                        } // for k
                        break;

                    
                    case  (e_gtype::AND):
                        //cout<<"it's AND"<<endl;
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 0){
                                np->val = 0;
                                break;
                            }
                            else np->val = 1;
                        } // for k
                        break;
                    
                    default: cout<<"it's not valid type !"<<endl;
                        

                    
                       
                } // swith type
            } // if lvl = j
            
        } // for i < Nnodes
    }// for j < max_lvl

     //using to debug
    // for(int i = 0; i < Nnodes; i++){
    //     np = Node[i];
    //     cout << np->num <<","<< np->val << endl;
    // }
    
    this->simulation_flag = 1;

}


void circuit::log_sim(void)
{
    for (int j = 1; j <= max_lvl; j++){ // lev = 1, 2 ... max_lev
        //cout<<"level is:"<<j<<endl;
        for(int i = 0; i < Nnodes; i++){
            np = Node[i];
            if (np->lvl == j){
                switch(np->type){
                        
                    case (e_gtype::BRCH):
                        np->val = np->unodes[0]->val;
                        //cout<<"it's branck"<<endl;
                        break;
                    
                    case (e_gtype::XOR):
                        sum = 0;
                        for (int k = 0 ; k < np->fin; k ++){
                            sum = sum ^ np->unodes[k]->val;
                        }
                            np->val = sum;
                        //cout<<"it's XOR"<<endl;
                        break;
                        
                    case  (e_gtype::OR):
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 1){
                                np->val = 1;
                                break;
                            }
                            else np->val = 0;
                        } // for k
                        //cout<<"it's OR"<<endl;
                        break;
                        
                    case  (e_gtype::NOR):
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 1){
                                np->val = 0;
                                break;
                            }
                            else np->val = 1;
                        } // for k
                        //cout<<"it's NOR"<<endl;
                        break;
                    
                    case  (e_gtype::NOT):
                        np->val = !(np->unodes[0]->val);
                        //cout<<"it's NOT"<<endl;
                        break;
       
                    case (e_gtype::NAND):
                        //cout<<"it's NAND"<<endl;
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 0){
                                //cout<<"unodes:"<<np->unodes[k]->num<<": "<<np->unodes[k]->val<<endl;
                                np->val = 1;

                                break;
                            }
                            else np->val = 0;
                        } // for k
                        break;

                    
                    case  (e_gtype::AND):
                        //cout<<"it's AND"<<endl;
                        for (int k = 0 ; k < np->fin; k ++){
                            if(np->unodes[k]->val == 0){
                                np->val = 0;
                                break;
                            }
                            else np->val = 1;
                        } // for k
                        break;
                    
                    default: cout<<"it's not valid type !"<<endl;
                        

                    
                       
                } // swith type
            } // if lvl = j
            
        } // for i < Nnodes
    }// for j < max_lvl
    this->simulation_flag = 1;

}

void circuit::log_sim_5(string log_file_name)
{
    char tmp;
    char logic_bit;

    int cnt_0, cnt_1, cnt_x, cnt_d, cnt_dn;

    //set val of all Node to -1 (initial value of each node in circuit is -1)
    for (int i = 0; i < Nnodes; i++){
        np = Node[i];
        np -> val = -1;
    }
    

    //read the primary input file, if number(in input file) = Pinput -> num, val = value(in input file)
    fin.open(log_file_name, ios::in);

    for(int i = 0; i < Nnodes; i++)
    {
        this->Node[i]->value_5 = logic_5_typedef::LOGIC_X;
    }
    
    while (getline(fin, log_in)) 
    {
        stringstream col(log_in);
       
        col>>log_num;
        col>>tmp;
        col>>logic_bit;
        logic_bit = isalpha(logic_bit) ? tolower(logic_bit) : logic_bit;
       // cout<<"log_num = "<<log_num<<endl;
        for(int i = 0; i < Npi; i++)
        {
            np = Pinput[i];
            //cout<<"num = "<<np->num<<endl;
            if(np->num == log_num)
            {
                switch(logic_bit)
                {
                    case '0':
                        np->value_5 = logic_5_typedef::LOGIC_0;
                        break;
                    case '1':
                        np->value_5 = logic_5_typedef::LOGIC_1;
                        break;
                    case 'x':
                        np->value_5 = logic_5_typedef::LOGIC_X;
                        break;
                    case 'd':
                        np->value_5 = logic_5_typedef::LOGIC_D;
                        break;
                    case 'n':
                        np->value_5 = logic_5_typedef::LOGIC_Dn;
                        break;
                    default:
                        printf("the char with ascii %d and char '%c' is not legal!\n", logic_bit, logic_bit);
                        break;                  
                }
                np->val = log_val;
                break;
               // cout << "Primary num and val is "<<np->num<<","<<np->val<<endl;
            }// if read file line successful
        } // for
    } // while
    fin.close();
   
    // find the max lvl from all Poutput
    int max_lvl = 0;
    int sum = 0; //in xor gate
    for (int i = 0; i < Npo; i++){
        np = Poutput[i];
        if (np->lvl > max_lvl){
            max_lvl = np->lvl;
        }
    }
    //cout<<"max_lvl = "<<max_lvl<<endl;
       
    // for all node which lvl is j (from 1 to max lvl)
        // if type = 1 (branch)  val = unodes[0] -> val
        // if type = 6 (NAND)  if undoes[i(from 0 to fin-1)]=0, val = 1, otherwise val =0 (control value = 0)
        // ...... other type
    for (int j = 1; j <= max_lvl; j++){ // lev = 1, 2 ... max_lev
        //cout<<"level is:"<<j<<endl;
        for(int i = 0; i < Nnodes; i++){
            np = Node[i];
            if (np->lvl == j){
                cnt_0 = cnt_1 = cnt_x = cnt_d = cnt_dn = 0;
                for(int k = 0; k < np->fin; k++)
                {
                    switch(np->unodes[k]->value_5)
                    {
                        case(logic_5_typedef::LOGIC_0):
                            cnt_0++;
                            break;
                        case(logic_5_typedef::LOGIC_1):
                            cnt_1++;
                            break;
                        case(logic_5_typedef::LOGIC_X):
                            cnt_x++;
                            break;
                        case(logic_5_typedef::LOGIC_D):
                            cnt_d++;
                            break;
                        case(logic_5_typedef::LOGIC_Dn):
                            cnt_dn++;
                            break;
                    }
                }

                switch(np->type){
                        
                    case (e_gtype::BRCH):
                        np->value_5 = np->unodes[0]->value_5;
                        //cout<<"it's branck"<<endl;
                        break;
                    
                    case (e_gtype::XOR):
                        if(cnt_x != 0)
                            np->value_5 = logic_5_typedef::LOGIC_X;
                        else
                        {
                            cnt_1 = cnt_1 % 2;
                            cnt_d = cnt_d % 2;
                            cnt_dn = cnt_dn % 2;
                            if((cnt_1 == 0) && (cnt_d == 0) && (cnt_dn == 0))
                                np->value_5 == logic_5_typedef::LOGIC_0;
                            else if((cnt_1 == 0) && (cnt_d == 0) && (cnt_dn == 1))
                                np->value_5 == logic_5_typedef::LOGIC_Dn;
                            else if((cnt_1 == 0) && (cnt_d == 1) && (cnt_dn == 0))
                                np->value_5 == logic_5_typedef::LOGIC_D;
                            else if((cnt_1 == 0) && (cnt_d == 1) && (cnt_dn == 1))
                                np->value_5 == logic_5_typedef::LOGIC_1;
                            else if((cnt_1 == 1) && (cnt_d == 0) && (cnt_dn == 0))
                                np->value_5 == logic_5_typedef::LOGIC_1;
                            else if((cnt_1 == 1) && (cnt_d == 0) && (cnt_dn == 1))
                                np->value_5 == logic_5_typedef::LOGIC_D;
                            else if((cnt_1 == 1) && (cnt_d == 1) && (cnt_dn == 0))
                                np->value_5 == logic_5_typedef::LOGIC_Dn;
                            else
                                np->value_5 == logic_5_typedef::LOGIC_0;
                        }
                        //cout<<"it's XOR"<<endl;
                        break;
                        
                    case  (e_gtype::OR):
                        if(cnt_1 != 0)
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        else if((cnt_d != 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        else if(cnt_x != 0)
                            np->value_5 = logic_5_typedef::LOGIC_X;
                        else if((cnt_d == 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_Dn;
                        else if((cnt_d != 0) && (cnt_dn == 0))
                            np->value_5 = logic_5_typedef::LOGIC_D;
                        else
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        break;
                        
                    case  (e_gtype::NOR):
                        if(cnt_1 != 0)
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        else if((cnt_d != 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        else if(cnt_x != 0)
                            np->value_5 = logic_5_typedef::LOGIC_X;
                        else if((cnt_d == 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_D;
                        else if((cnt_d != 0) && (cnt_dn == 0))
                            np->value_5 = logic_5_typedef::LOGIC_Dn;
                        else
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        break;
                        //cout<<"it's NOR"<<endl;
                    
                    case  (e_gtype::NOT):
                        switch(np->unodes[0]->value_5)
                        {
                            case(logic_5_typedef::LOGIC_0):
                                np->value_5 = logic_5_typedef::LOGIC_1;
                                break;
                            case(logic_5_typedef::LOGIC_1):
                                np->value_5 = logic_5_typedef::LOGIC_0;
                                break;
                            case(logic_5_typedef::LOGIC_D):
                                np->value_5 = logic_5_typedef::LOGIC_Dn;
                                break;
                            case(logic_5_typedef::LOGIC_Dn):
                                np->value_5 = logic_5_typedef::LOGIC_D;
                                break;
                            case(logic_5_typedef::LOGIC_X):
                                np->value_5 = logic_5_typedef::LOGIC_X;
                                break;
                        }
                        //cout<<"it's NOT"<<endl;
                        break;
       
                    case (e_gtype::NAND):
                        //cout<<"it's NAND"<<endl;
                        if(cnt_0 != 0)
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        else if((cnt_d != 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        else if(cnt_x != 0)
                            np->value_5 = logic_5_typedef::LOGIC_X;
                        else if((cnt_d != 0) && (cnt_dn == 0))
                            np->value_5 = logic_5_typedef::LOGIC_Dn;
                        else if((cnt_d == 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_D;
                        else
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        break;

                    
                    case  (e_gtype::AND):
                        //cout<<"it's AND"<<endl;
                        if(cnt_0 != 0)
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        else if((cnt_d != 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_0;
                        else if(cnt_x != 0)
                            np->value_5 = logic_5_typedef::LOGIC_X;
                        else if((cnt_d != 0) && (cnt_dn == 0))
                            np->value_5 = logic_5_typedef::LOGIC_D;
                        else if((cnt_d == 0) && (cnt_dn != 0))
                            np->value_5 = logic_5_typedef::LOGIC_Dn;
                        else
                            np->value_5 = logic_5_typedef::LOGIC_1;
                        break;
                    
                    default: cout<<"it's not valid type !"<<endl;
                        

                    
                       
                } // swith type
            } // if lvl = j
            
        } // for i < Nnodes
    }// for j < max_lvl

     //using to debug
    // for(int i = 0; i < Nnodes; i++){
    //     np = Node[i];
    //     cout << np->num <<","<< np->val << endl;
    // }
    
    this->simulation_flag = 1;

}
